...
# Patches: 1) HashMap compatibility for Keycloak 26.5+, 2) Build magic link from action URL (with execution)
RUN git clone --branch magic-link https://github.com/front-matter/keycloak-2fa-email-authenticator.git && \
  ...
  sed -i 's/UriBuilder\.fromUri(context\.getUriInfo()\.getRequestUri())/UriBuilder.fromUri(context.getActionUrl(context.getExecution().getId()))/g' src/main/java/com/mesutpiskin/keycloak/auth/email/EmailAuthenticatorForm.java && \
